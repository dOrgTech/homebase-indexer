...
async def on_factory_origination(
    ...
    try:
        ...
        # Check if it's a TzktOrigination wrapper by checking if it has 'data' attribute
        if hasattr(registry_origination, 'data') and registry_origination.data is not None:
            ...
            # TzktOperationData doesn't have .data, it IS the data
            originated_contract = cast(str, registry_origination.originated_contract_address)
            origination_level = registry_origination.level
